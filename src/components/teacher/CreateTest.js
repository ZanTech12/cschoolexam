...
import { testsAPI, classesAPI, subjectsAPI, questionSetsAPI, dashboardAPI, getAuthData } from '../../api';
import Loading from '../common/Loading';

const ManageTests = () => {
  const queryClient = useQueryClient();
  
  // Using getAuthData() logic from CreateTest
  ...
  // Fetch teacher assignments ONLY if the user is a teacher
  ...
  // Extract only this teacher's assignments
  ...
  // Map classes to their allowed subjects for this teacher
  const classSubjectMap = useMemo(() => {
    ...
    myAssignments.forEach((assignment) => {
      ...
      if (classId && subjectId) {
        ...
        if (!map[classId].includes(subjectId)) map[classId].push(subjectId);
      }
    });
    return map;
  }, [myAssignments]);

  // Filter classes dropdown for teachers
  const myClasses = useMemo(() => {
    if (!classes?.data) return [];
    if (!isTeacher) return classes.data; // Admin sees all
    
    const allowedClassIds = [...new Set(myAssignments.map(a => a.class_id?._id || a.classId?._id).filter(Boolean))];
    return classes.data.filter((cls) => allowedClassIds.includes(cls._id));
  }, [classes, isTeacher, myAssignments]);

  // Filter subjects dropdown based on selected class (for teachers)
  const mySubjectsForForm = useMemo(() => {
    if (!subjects?.data) return [];
    if (!isTeacher) return subjects.data; // Admin sees all
    
    if (!formData.classId) {
      // No class selected yet, show all subjects assigned to teacher across any class
      const allowedSubjectIds = [...new Set(myAssignments.map(a => a.subject_id?._id || a.subjectId?._id).filter(Boolean))];
      return subjects.data.filter((s) => allowedSubjectIds.includes(s._id));
    }
    
    // Class selected, show ONLY subjects assigned to THIS class
  ...
  }, [subjects, isTeacher, formData.classId, classSubjectMap, myAssignments]);

  // Filter tests table for teachers
  ...
  const handleChange = (e) => {
    ...
    // If class changes, reset subject and questions because allowed subjects will change
    if (name === 'classId') {
      setFormData({ 
        ...formData, 
        [name]: value, 
  ...
  const handleOpenCreate = () => {
    if (isTeacher && myAssignments.length === 0) {
      setError('You have no class/subject assignments. Contact admin.');
      return;
    }
    ...
    setFormData({
      ...
      startDate: '', endDate: '', passMark: 50,
      instructions: '', questions: [], questionSetId: '',
    });
    setSelectedQuestionSet('');
    setError('');
  ...
  const handleOpenEdit = (test) => {
    if (isTeacher) {
      ...
      if (!canEdit) return alert('You do not have permission to edit this test.');
    }
  ...
  const handleDelete = (test) => {
    if (isTeacher) {
      ...
      if (!canDelete) return alert('You do not have permission to delete this test.');
    }
  ...
  if (testsLoading || classesLoading || subjectsLoading || assignmentsLoading) {
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '1.5rem' }}>
        <div>
          ...
          <p className="page-subtitle">
            {isTeacher 
              ? `Manage tests for your assigned classes and subjects` 
              : 'Manage all tests'}
          </p>
        </div>
      ...
      {isTeacher && (
        <div className="card" style={{ marginBottom: '1rem', padding: '1rem 1.5rem' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
            ...
            <div>
              <strong>Your Assigned Classes & Subjects:</strong>
              {myAssignments.length === 0 ? (
                <span style={{ color: '#dc3545', marginLeft: '0.5rem' }}>
                  No assignments found. Contact your administrator.
                </span>
              ) : (
                <span style={{ marginLeft: '0.5rem' }}>
                  {myAssignments.map((a, idx) => (
                    <span key={a.id || idx} className="badge badge-info" style={{ marginRight: '0.25rem', marginBottom: '0.25rem' }}>
                ...
                {myTests?.length === 0 ? (
                  <tr>
                    <td colSpan="10" style={{ textAlign: 'center', padding: '2rem' }}>
                      {isTeacher ? 'No tests found for your assigned classes and subjects' : 'No tests found'}
                    </td>
                  </tr>
                ) : (
                  myTests?.map((test) => {
                      ...
                      <tr key={test._id}>
                        ...
                        <td>
                          <div className="btn-group">
                            <button className="btn btn-info btn-sm" onClick={() => handleOpenEdit(test)}>Edit</button>
                            {test.resultsPublished ? (
                              <button className="btn btn-warning btn-sm" onClick={() => handleUnpublish(test._id)}>Unpublish</button>
                            ) : (
                              <button className="btn btn-success btn-sm" onClick={() => handlePublish(test._id)}>Publish</button>
                            )}
        ...
        <div className="modal-overlay" onClick={handleCloseModal}>
          <div className="modal modal-lg" onClick={(e) => e.stopPropagation()}>
            ...
            <div 
              ...
              style={{ position: 'relative', overflowY: 'auto', maxHeight: 'calc(100vh - 150px)' }}
            >
              ...
              <form onSubmit={handleSubmit}>
                ...
                <div className="form-row">
                  <div className="form-group">
                    ...
                    <select name="classId" className="form-control form-select" value={formData.classId} onChange={handleChange} required>
                    ...
                    {isTeacher && myClasses.length === 0 && (
                      <span className="form-text" style={{ color: '#dc3545' }}>No classes assigned to you.</span>
                    )}
                  </div>
                  ...
                  <div className="form-group">
                    ...
                    <select name="subjectId" className="form-control form-select" value={formData.subjectId} onChange={handleChange} required disabled={!formData.classId}>
                    ...
                    {isTeacher && formData.classId && mySubjectsForForm.length === 0 && (
                      <span className="form-text" style={{ color: '#dc3545' }}>No subjects assigned to you for this class.</span>
                    )}
                  </div>
                </div>
                ...
                <div className="form-row-3">
                  <div className="form-group">
                    ...
                    <input type="number" name="duration" className="form-control" value={formData.duration} onChange={handleChange} min="1" required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Pass Mark (%) *</label>
                    <input type="number" name="passMark" className="form-control" value={formData.passMark} onChange={handleChange} min="0" max="100" required />
                  </div>
                  <div className="form-group">
                    <label className="form-label">Questions</label>
                    <input type="text" className="form-control" value={formData.questions?.length || 0} disabled />
                  </div>
                </div>
                ...
                <div className="form-group">
                  ...
                  <select className="form-control form-select" value={selectedQuestionSet} onChange={handleQuestionSetSelect} disabled={!formData.classId || !formData.subjectId}>
                    <option value="">-- Select Question Set --</option>
                    {questionSets?.data?.map((qs) => (
                      <option key={qs._id} value={qs._id}>{qs.title} ({qs.questions?.length} questions)</option>
                    ))}
                  </select>
                ...
                <div className="form-group">
                  ...
                  <textarea name="instructions" className="form-control" value={formData.instructions} onChange={handleChange} rows="3" placeholder="Enter any special instructions for students..." />
                </div>
              ...
              {showModalScrollBtn && (
                <button 
                  ...
                  style={{
                    ...
                    color: '#ffffff', border: 'none', padding: '8px 14px', borderRadius: '50px',
                    fontSize: '0.8rem', fontWeight: '600', cursor: 'pointer',
                    boxShadow: '0 8px 20px rgba(99, 102, 241, 0.4)', zIndex: 20,
                    display: 'flex', alignItems: 'center', gap: '6px',
                    transition: 'transform 0.2s ease, box-shadow 0.2s ease'
                  }}
                  onMouseEnter={(e) => { e.currentTarget.style.transform = 'translateY(-2px)'; e.currentTarget.style.boxShadow = '0 10px 25px rgba(99, 102, 241, 0.5)'; }}
