...
const TeacherDashboard = () => {
  ...
  const { data: caStats, isLoading: loadingCA } = useQuery({
    ...
    enabled: !!user?._id,
    // Keep data fresh so the number updates after teacher enters scores
    staleTime: 30 * 1000,
    refetchOnWindowFocus: true,
  });

  /* ── Fallback: fetch submissions to derive count if dedicated endpoint fails ── */
  ...
  /* ── Derive CA subjects entered count — with robust fallback ── */
  const caSubjectsEntered = React.useMemo(() => {
    // Primary: use dedicated endpoint
    if (caStats?.success && caStats?.data != null) {
      // ✅ UPDATED: Use completedSubjects first (new backend format), then fall back to legacy names
      const count = 
    ...
    // Fallback: derive from submissions list
  ...
  return (
    <div className="td-wrapper">
      ...
      {/* ── CA Entry Reminder (shown when not all subjects are entered) ── */}
