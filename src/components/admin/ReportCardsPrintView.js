...
const StudentReportCard = () => {
  ...
  // ==========================================
  // TERM DATES & DURATION CALCULATION
  // ==========================================
  const termStartDate = report?.term?.startDate || null;
  const termEndDate = report?.term?.endDate || null;
  const nextTermBegins = report?.term?.nextTermBegins || null;
  ...
  const termDuration = useMemo(() => {
    ...
    return {
      ...
      display: diffWeeks > 0 
        ? `${diffWeeks} week${diffWeeks > 1 ? 's' : ''}${remainingDays > 0 ? ` ${remainingDays} day${remainingDays > 1 ? 's' : ''}` : ''}`
        : `${diffDays} day${diffDays !== 1 ? 's' : ''}`
    };
  }, [termStartDate, termEndDate]);

  // Calculate days until next term begins
  const daysUntilNextTerm = useMemo(() => {
    if (!nextTermBegins) return null;
    
    const now = new Date();
    const nextTerm = new Date(nextTermBegins);
    const diffTime = nextTerm - now;
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
  ...
  const attendancePercentage = useMemo(() => {
    const present = timesPresent !== '' ? Number(timesPresent) : null;
    const open = timesSchoolOpen !== '' ? Number(timesSchoolOpen) : null;
    
    if (present === null || open === null || open === 0) return null;
    return ((present / open) * 100).toFixed(1);
  }, [timesPresent, timesSchoolOpen]);
  ...
  return (
    <div className="report-sheet-wrapper">
      <style>{`
        ...
        .bio-term-dates .bio-item {
          flex: 1;
          min-width: 140px;
        }
        .bio-term-dates .date-range {
          font-size: 0.8em;
          color: #444;
        }
        ...
        .attendance-percentage-badge.good {
          background-color: #d4edda;
          color: #155724;
        }
        .attendance-percentage-badge.fair {
          background-color: #fff3cd;
          color: #856404;
        }
        .attendance-percentage-badge.poor {
      ...
      <div className="a4-document">
        ...
        <div className="bio-data-section">
          ...
          {/* Term Duration Row */}
          {termStartDate && termEndDate && (
            <div className="bio-term-dates" style={{ marginTop: '8px', paddingTop: '8px', borderTop: '1px dashed #ddd' }}>
              <div className="bio-item">
                <span className="bio-label">Term Started</span>
                <span className="bio-value date-range">{formatDate(termStartDate)}</span>
              </div>
              <div className="bio-item">
                <span className="bio-label">Term Ended</span>
                <span className="bio-value date-range">{formatDate(termEndDate)}</span>
              </div>
              {termDuration && (
                <div className="bio-item">
                  <span className="bio-label">Term Duration</span>
                  <span className="bio-value date-range">{termDuration.display}</span>
                </div>
              )}
            </div>
          )}
              ...
              <span className="attendance-value present">
                ...
                {attendancePercentage !== null && (
                  <span className={`attendance-percentage-badge ${
                    ...
                    attendancePercentage >= 50 ? 'fair' : 'poor'
                  }`}>
        ...
        <footer className="sheet-footer-elegant">
          <div className="footer-dates-grid">
            <div className="footer-date-item">
              ...
              <span className="fd-value">{formatDate(termStartDate)}</span>
            </div>
            <div className="footer-date-item">
              <span className="fd-label">Term Ends:</span>
              <span className="fd-value">{formatDate(termEndDate)}</span>
            </div>
            {termDuration && (
              <div className="footer-date-item">
                <span className="fd-label">Duration:</span>
                <span className="fd-value">{termDuration.display}</span>
              </div>
            )}
          </div>
          ...
          {nextTermBegins && (
            <div className="next-term-highlight">
              ...
              <span className="nt-date">{formatDate(nextTermBegins)}</span>
              {/* Countdown until next term */}
              {daysUntilNextTerm !== null && daysUntilNextTerm > 0 && (
                <span className="next-term-countdown">
