    ...
    <div className="dash">
      ...
      <section className="dash-section">
        ...
        <div className="dash-actions">
          ...
          <ActionLink to="/admin/broadsheet" icon={Icons.classes} label="View Broadsheet" accent="#f59e0b" />
        </div>
      </section>
      ...
      {!broadsheetStatsLoading && (passedResults > 0 || failedResults > 0) && (
        <section className="dash-section">
          <div className="dash-section__head">
            <h2 className="dash-section__title">Broadsheet Overview</h2>
            <Link to="/admin/broadsheet" className="dash-section__link">View Full Broadsheet {Icons.arrowRight}</Link>
          </div>
          ...
          {broadsheetStatsData?.termInfo && (
            <div style={{ marginTop: '12px', padding: '10px 16px', background: 'rgba(99, 102, 241, 0.08)', borderRadius: '8px', fontSize: '13px', color: '#64748b', display: 'flex', gap: '24px' }}>
              <span><strong>Term:</strong> {broadsheetStatsData.termInfo.name}</span>
              <span><strong>Session:</strong> {broadsheetStatsData.sessionInfo.name}</span>
              <span><strong>Assessed:</strong> {broadsheetStats.assessedStudents} of {broadsheetStats.totalStudents} students</span>
            </div>
          )}

          {allResults.length > 0 && (
            <div className="dash-table-wrap" style={{ marginTop: '20px' }}>
              <h3 style={{ fontSize: '14px', color: '#64748b', marginBottom: '8px' }}>Recent Online Test Submissions</h3>
              <div className="dash-table-scroll">
                <table className="dash-table">
                  <thead>
                    <tr><th>Student</th><th>Test</th><th>Score</th><th>Status</th></tr>
                  </thead>
                  <tbody>
                    {allResults.slice(0, 5).map((r, i) => (
                      <tr key={r._id || i}>
                        ...
                        <td><strong>{r.percentage || 0}%</strong> <span className="dash-table__muted">({r.score}/{r.totalQuestions || '?'})</span></td>
        ...
        <section className="dash-section">
          ...
          <div className="dash-table-wrap">
            <div className="dash-table-scroll">
              <table className="dash-table">
                <thead>
                  <tr><th>Title</th><th>Class</th><th>Subject</th><th>Questions</th><th>Created By</th></tr>
                </thead>
                <tbody>
                  {questionSetsData.data.slice(0, 5).map((qs) => (
                    <tr key={qs._id}>
                      ...
                      <td><span className="dash-badge">{qs.questions?.length || 0}</span></td>
